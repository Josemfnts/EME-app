...
:root{
  ...
  --purple:#D4A847;--purple-l:#E8C96A;--purple-d:#B8912E;
  --purple-bg:rgba(212,168,71,0.12);--purple-bg2:rgba(212,168,71,0.07);
...
input:focus,select:focus,textarea:focus{outline:none;border-color:var(--purple)!important;box-shadow:0 0 0 3px rgba(212,168,71,0.12)!important}
...
function Bt({children,onClick,disabled,full,variant='primary',small,style:sx,...rest}) {
  ...
  return <button onClick={disabled?undefined:onClick} style={{
    ...
    width:full?'100%':'auto',
    color:p?(disabled?'var(--text3)':'#0F0F0F'):d?'var(--red)':'var(--purple)',
    ...
    boxShadow:p&&!disabled?'0 4px 16px rgba(212,168,71,0.38)':'none',
    ...sx
...
function Modal({children}) {
  return <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(6px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:20}}>
...
const HexSVG = ({size=28,color='#0F0F0F'}) => <svg width={size} height={size} viewBox="0 0 30 30" fill="none">
  <polygon points="15,2 26.5,8.5 26.5,21.5 15,28 3.5,21.5 3.5,8.5" stroke={color} strokeWidth="1.8" fill="none"/>
  <polygon points="15,8 21.5,11.75 21.5,19.25 15,23 8.5,19.25 8.5,11.75" stroke={color} strokeWidth="1" strokeOpacity="0.55" fill="none"/>
</svg>
...
function SvcCard({s,sel,onClick,i,bookBtn}) {
  return <div onClick={onClick} className={`anim d${(i%5)+1}`} style={{
    ...
    boxShadow:sel?'0 12px 28px rgba(212,168,71,0.28)':'var(--shadow)',
...
function PWAPrompt({ onClose }) {
  ...
  return (
    <div style={{ position:'fixed', bottom:0, left:'50%', transform:'translateX(-50%)', width:'100%', maxWidth:480, zIndex:300, padding:'0 16px 16px' }}>
      <div className="scale-in" style={{ background:'var(--white)', borderRadius:20, padding:20, boxShadow:'0 -4px 32px rgba(212,168,71,0.18)', border:'1.5px solid var(--border)' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:16 }}>
          <div style={{ width:48, height:48, borderRadius:14, background:'linear-gradient(135deg,var(--purple),var(--purple-l))', display:'flex', alignItems:'center', justifyContent:'center' }}>
            <HexSVG size={26} color="#0F0F0F"/>
          </div>
          <div>
...
function Landing({svcs,stys,user,isA,onRes,onLog,onAcc,onAdm,salonConfig}) {
  ...
  const addr=salonConfig?.address||'—'
  ...
  return <div style={{paddingBottom:88}}>
    <div style={{position:'relative',height:260,overflow:'hidden',background:'#1A1A1A'}}>
      <img src={HERO} alt="" style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:'center 15%'}} onError={e=>{e.target.style.display='none';e.target.parentElement.style.background='#1A1A1A'}}/>
    ...
    <div style={{background:'var(--white)',padding:'20px 20px 16px',borderBottom:'1px solid var(--border)'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
        <div>
          ...
          <h1 style={{fontSize:36,fontWeight:900,color:'var(--text)',letterSpacing:-2.5,lineHeight:1,marginBottom:3}}>EME BS</h1>
          <p style={{fontSize:11,fontWeight:700,color:'var(--purple)',letterSpacing:2.5,textTransform:'uppercase',marginBottom:4}}>Barber Studio</p>
          <p style={{fontSize:12,color:'var(--text3)'}}>Barbería</p>
        </div>
        <div style={{width:54,height:54,borderRadius:16,background:'linear-gradient(135deg,var(--purple),var(--purple-l))',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 6px 20px rgba(212,168,71,0.38)',flexShrink:0}}>
          <HexSVG size={30} color="#0F0F0F"/>
        </div>
      </div>
    </div>
    ...
    <div style={{position:'fixed',bottom:0,left:'50%',transform:'translateX(-50%)',width:'100%',maxWidth:480,background:'rgba(26,26,26,0.96)',backdropFilter:'blur(14px)',borderTop:'1px solid var(--border)',padding:'12px 20px 18px',zIndex:50}}>
      <button onClick={()=>onRes(null)} style={{width:'100%',padding:15,fontSize:15,fontWeight:700,color:'#0F0F0F',background:'linear-gradient(135deg,var(--purple),var(--purple-l))',border:'none',borderRadius:14,cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',justifyContent:'center',gap:8,boxShadow:'0 6px 20px rgba(212,168,71,0.42)'}}>
        Reservar cita
...
function Auth({onLogin,onBack}) {
  ...
  return <div style={{maxWidth:480,margin:'0 auto',minHeight:'100vh',background:'var(--white)'}}>
    ...
    <div style={{padding:'36px 28px 24px',textAlign:'center'}}>
      <div style={{width:62,height:62,borderRadius:20,background:'linear-gradient(135deg,var(--purple),var(--purple-l))',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 18px',boxShadow:'0 6px 22px rgba(212,168,71,0.38)'}}>
        <HexSVG size={32} color="#0F0F0F"/>
      </div>
      <h1 style={{fontSize:24,fontWeight:900,marginBottom:4,letterSpacing:-1,color:'var(--text)'}}>EME Barber Studio</h1>
...
setSlots(unionSlots)
  ...
  return <div style={{paddingBottom:110}}>
    ...
    {step===2&&<div style={{background:'var(--white)',padding:20}}>
      ...
      <div style={{display:'flex',gap:12,overflowX:'auto',paddingBottom:6}}>
        {(time?stys.filter(s=>getSlotsForDay(date,s.id,schedules,dayData.bd,dayData.bl,svc?.duration||30).includes(time)):stys).map(s=>{const sl=sty?.id===s.id;return<button key={s.id} onClick={()=>setSty(s)} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:8,minWidth:80,background:'none',border:'none',cursor:'pointer',padding:'8px 4px',flexShrink:0}}>
          <div style={{width:64,height:64,borderRadius:32,background:'var(--purple-bg2)',border:sl?'3px solid var(--purple)':'2px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,fontWeight:700,color:'var(--purple)',overflow:'hidden',transition:'all .2s',boxShadow:sl?'0 4px 16px rgba(212,168,71,0.32)':'none'}}>
      ...
      {date&&<div style={{background:'var(--white)',borderRadius:18,border:'1.5px solid var(--border)',padding:16,marginBottom:12,boxShadow:'var(--shadow)'}}>
        ...
        {sL?<Sp/>:slots.length===0?<Em icon="😔" text="Sin horarios disponibles este día"/>:
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8}}>
            {slots.map(s=><button key={s} onClick={()=>setTime(s)} style={{padding:'10px 6px',borderRadius:12,border:time===s?'none':'1.5px solid var(--border)',background:time===s?'linear-gradient(135deg,var(--purple),var(--purple-l))':'var(--white)',color:time===s?'#fff':'var(--text)',fontSize:13,fontWeight:time===s?700:500,cursor:'pointer',fontFamily:'inherit',boxShadow:time===s?'0 4px 12px rgba(212,168,71,0.30)':'none',transition:'all .15s'}}>{s}</button>)}
          </div>}
      </div>}
    ...
    <div style={{position:'fixed',bottom:0,left:'50%',transform:'translateX(-50%)',width:'100%',maxWidth:480,background:'rgba(26,26,26,0.96)',backdropFilter:'blur(14px)',borderTop:'1px solid var(--border)',padding:'12px 20px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',zIndex:50}}>
  ...
  return <div>
    ...
    <div style={{padding:'8px 20px 20px',background:'var(--white)',borderBottom:'1px solid var(--border)'}}>
      <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:16}}>
        <div style={{width:52,height:52,borderRadius:16,background:'linear-gradient(135deg,var(--purple),var(--purple-l))',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,fontWeight:800,color:'#fff',boxShadow:'0 4px 14px rgba(212,168,71,0.32)'}}>{ini}</div>
        <div style={{flex:1}}>
...
function SalonConfigModal({config,onSave,onClose}) {
  ...
  return <Modal>
    ...
    <div style={{marginBottom:14}}>
      ...
      <textarea value={addr} onChange={e=>setAddr(e.target.value)} rows={2} placeholder="Calle Ejemplo, 1&#10;Ciudad" style={{width:'100%',padding:'12px 14px',fontSize:14,border:'1px solid var(--border2)',borderRadius:12,background:'var(--white)',color:'var(--text)',fontFamily:'inherit'}}/>
    </div>
    <In label="Teléfono" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="+34 976 XXX XXX"/>
    <In label="Instagram" value={insta} onChange={e=>setInsta(e.target.value)} placeholder="@eme.barber.studio"/>
...
function SvcModal({data,onSave,onClose}) {
  ...
  return <Modal>
    ...
    <In label="Nombre" required value={name} onChange={e=>setName(e.target.value)} placeholder="Ej: CORTE EME BS"/>
...
function StyModal({data,onSave,onClose}) {
  ...
  return <Modal>
    ...
    <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
      ...
      <button onClick={()=>setActive(!active)} style={{width:44,height:24,borderRadius:12,position:'relative',cursor:'pointer',border:'none',background:active?'var(--purple)':'var(--border)',transition:'all .3s',boxShadow:active?'0 2px 8px rgba(212,168,71,0.35)':'none'}}>
...
function Admin({user,onBack,onDataChanged,salonConfig,onSalonConfigChanged}) {
  const LS_KEY='eme-bs-admin-stylist'
    ...
    <div style={{padding:'14px 16px',background:'var(--white)',borderBottom:'1px solid var(--border)'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <div style={{width:36,height:36,borderRadius:10,background:'linear-gradient(135deg,var(--purple),var(--purple-l))',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 3px 10px rgba(212,168,71,0.35)'}}>
            <HexSVG size={20}/>
          </div>
          <div>
            <div style={{fontSize:16,fontWeight:800,color:'var(--text)'}}>Panel Admin</div>
            <div style={{fontSize:10,color:'var(--text3)',letterSpacing:0.3}}>EME Barber Studio</div>
          </div>
        </div>
      ...
      <button onClick={()=>setShowStylistPicker(true)} style={{width:'100%',display:'flex',alignItems:'center',gap:10,padding:'10px 14px',background:myStylist?'linear-gradient(135deg,var(--purple),var(--purple-l))':'var(--bg)',border:myStylist?'none':'1.5px dashed var(--border2)',borderRadius:12,cursor:'pointer',transition:'all .2s',boxShadow:myStylist?'0 4px 14px rgba(212,168,71,0.28)':'none'}}>
        {myStylist?<>
    ...
    <div style={{padding:16}}>
        ...
        <div style={{background:'var(--white)',borderRadius:16,border:'1.5px solid var(--border)',padding:14,marginBottom:16,boxShadow:'var(--shadow)'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:2}}>
            ...
            {days.map((d,i)=>{
              ...
              return<button key={toK(d)} onClick={()=>setSd(d)} style={{height:30,borderRadius:15,background:sl?'linear-gradient(135deg,var(--purple),var(--purple-l))':'transparent',border:'none',cursor:'pointer',fontSize:11,fontWeight:sl||isT(d)?700:400,color:sl?'#fff':isT(d)?'var(--purple)':'var(--text)',boxShadow:sl?'0 2px 8px rgba(212,168,71,0.3)':'none'}}>{d.getDate()}</button>
            })}
          </div>
        </div>
      ...
      {tab==='config'&&<div>
        <div style={{background:'var(--white)',borderRadius:16,border:'1.5px solid var(--border)',padding:16,marginBottom:12,boxShadow:'var(--shadow)'}}>
          ...
          {[{l:'Nombre',v:'EME Barber Studio'},{l:'Dirección',v:salonConfig?.address||'—'},{l:'Teléfono',v:salonConfig?.phone||'—'},{l:'Instagram',v:salonConfig?.instagram||'—'}].map(({l,v})=>
...
if ((isIOS || isAndroid) && !alreadyPrompted) {
  ...
  return <div style={{maxWidth:480,margin:'0 auto',minHeight:'100vh',background:'var(--bg)',boxShadow:'0 0 60px rgba(212,168,71,0.04)'}}>
    <style>{CSS}</style>
